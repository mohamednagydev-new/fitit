...
export async function buildPartnerReport(partnerId: string, month?: string): Promise<PartnerReport | null> {
  ...
  const lines = [
    `📊 ${partner.name} report — ${key}`,
    `👀 Page views (all-time): ${partner.views} · contact taps: ${partner.contacts}`,
    `📩 Leads this month: ${leads}`,
  ];
  if (isGym) {
    lines.push(
      `🆕 New members via your code: ${newMembers} (total: ${membersTotal}, active last 30d: ${active30})`,
      `💪 Member workouts this month: ${memberWorkouts}`,
    );
    if (topPerformers.length) {
      lines.push(`🏆 Top performers: ${topPerformers.map((t) => `${t.firstName} (${t.workouts})`).join(' · ')}`);
    }
  }
  lines.push('— FIT IT');
