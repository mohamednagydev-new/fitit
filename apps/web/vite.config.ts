...
export default defineConfig(({ mode }) => {
  // Brand comes from .env / .env.<mode> (e.g. `vite build --mode fitit`).
  // Exposed onto process.env so tailwind.config.js (which postcss loads inside
  // this same process) can branch its palette on the brand too.
  ...
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        ...
        id: '/',
        name: isPulse ? 'PULSE — A Fresh Start To Get Healthy' : `${NAME} — Train hard, eat right, win with friends`,
        short_name: NAME,
        description: 'Coach-led workouts, healthy recipes, challenges and a wellness library.',
        theme_color: THEME,
